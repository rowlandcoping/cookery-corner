...
function getPost($slug)
{
	...
	$sql = $conn->prepare("SELECT * FROM liveblog WHERE slug=?");
...
function displayYears($collapsed, $current_year, $blog_ID, $article_type) {
	...
	echo "<li class=\"year\">";
	if ($collapsed == TRUE) {
		echo "<a class=\"fa fa-angle-double-right\" href=\"/liveblog.php?year=".$current_year; 
		} else {
		echo "<a class=\"fa fa-angle-double-down\" href=\"/liveblog.php?collapse=?";
		}	
...
function displayMonths ($collapsed, $hidden, $month, $year, $blog_ID) {
	...
	if ($hidden==TRUE) {echo "<li class=\"hidden\"><a class=\"fa fa-angle-double-right\" 
				href=\"/liveblog.php?year=".$year."&ID=".$blog_ID."\"></a>&nbsp".$month;
	} elseif ($collapsed==TRUE) {echo "<li class=\"month\"><a class=\"fa fa-angle-double-right\"
				href=\"/liveblog.php?year=".$year."&month=".$month."&ID=".$blog_ID."\"></a>&nbsp".$month;
	} else {echo "<li class=\"month\"><a class=\"fa fa-angle-double-down\"
				href=\"/liveblog.php?year=".$year."&ID=".$blog_ID."\"></a>&nbsp".$month;
	}
	echo "<ul class=\"articles\">";
	$select_article=$conn->prepare("SELECT liveblog_ID, date FROM article_index WHERE month_full=? AND year=?");
	...
	foreach ($article_info as $article) {
		$article_ID=$article['liveblog_ID'];
		$get_content =$conn->prepare("SELECT title, slug FROM liveblog WHERE ID=?");
		...
		echo "<a href=\"/liveblog/".$c['slug']."\">".$date_res['date'].": ".$c['title']."</a></li>";
		}					
	}
echo "</ul></li>";
}
