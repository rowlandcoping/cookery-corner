...
$blogpath = "$path"."/assets/images/blog";
$searchblog = "$path"."/admin/includes/search_blog.php";
$imgprocess = "$path"."/admin/includes/imageprocess.php";

//paths to blog includes
$editpost= "$path"."/admin/includes/blog_update/edit_post.php";
$publishpost= "$path"."/admin/includes/blog_update/publish_post.php";
//
$basicupdate = "$path"."/admin/includes/blog_update/basic_update.php";
$contentupdate = "$path"."/admin/includes/blog_update/content_update.php";
//
$basicactions = "$path"."/admin/includes/blog_update/basic_actions.php";
$contentactions = "$path"."/admin/includes/blog_update/content_actions.php";
...
require_once($searchblog);
$listing="Search Results for \"".$_POST['search']."\":";

}else{
	
$ID=$_SESSION['ID'];
$stmt=$conn->prepare("SELECT * from blog ORDER BY timestamp_created DESC");
$stmt ->execute();
...
echo"<h1>Blog Update Tool</h1><hr>";
...
foreach ($result as $r) { 
echo	"<tr>
		<td><a href=\"/blog/".$r['slug']."\"<h2>".$r['title']."</h2></a></td>";
echo 	"<td><a class=\"fa fa-pencil btn edit\" href=\"update_blog.php?edit-blog=".$r['ID']."\"></a></td>
		</tr>";
}}
...
<input type="submit" class="button" name="showtell" value="List Blog Posts" />
</form>
<hr>
