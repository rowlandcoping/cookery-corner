...
<p>Welcome to the fantastical home of Random Reviews, the beating heart of this website for the last 20 years.</p>
<p>These pages have historically been home to eye-aching colour schemes, head-scratching content and unparalleled artistic bravery.</p>
<p>Plus a lot of text.  Too much text, some would say.</p>
<p>At present you can find every review ever on this page, because there are only 
<?php
...
?>
 of them - 
but don't worry, I'll do a search thingy in the unlikely event things ever get out of hand.</p>
...
if (!empty ($rows['main_image'])) {$main=$rows['main_image'];}else{$main=$rows['image1'];}


echo"<div class=\"overview\"style=\"";
 
if (!empty ($rows['page_background'])) {echo "background-image: url(/assets/backgrounds/reviews/".$rows['page_background'].")";}
else {echo "background-color: ".$rows['page_color'];}
echo"; border-color: ".$rows['border_text']."; color: ".$rows['border_text']."\">
<div class=\"smallcont\">	
<div class=\"smallimg\">
...
if (!empty ($rows['text_background'])) {echo "background-image: url(/assets/backgrounds/reviews/".$rows['text_background'].")";}
else {echo "background-color: ".$rows['textback_color'];}
	
echo"\">
<h2><a href=\"/reviews/".$rows['slug']."\" style=\"color:".$rows['border_text']."\"
onmouseover=\"this.style.color = '".$rows['h2h3_color']."'\"onmouseout=\"this.style.textDecoration = 'none'; this.style.color='".$rows['border_text']."'\">
 ".$rows['title']."</a></h2>
